    ...
    <div className="w-full lg:w-72 shrink-0 flex flex-col gap-6">
      ...
      <div className="flex items-center justify-between border-b border-white/5 pb-4">
        <h3 className="text-sm font-black text-white uppercase tracking-widest flex items-center gap-2">
          Filtrar Álbumes <SlidersHorizontal className="w-4 h-4 text-emerald-500" />
        </h3>
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className="relative">
          ...
          <input 
            ...
            className="w-full bg-[#09090b] border border-white/10 rounded-xl pl-9 pr-4 py-2 text-sm text-white focus:outline-none focus:border-emerald-500 transition-colors"
          />
        </div>
              ...
              <label key={col.id} className="flex items-center justify-between cursor-pointer group" onClick={() => toggleCollection(col.id)}>
                <div className="flex items-center gap-3">
                  <div className={`w-4 h-4 rounded-[4px] border flex items-center justify-center transition-colors ${
                    isChecked ? 'bg-emerald-600 border-emerald-600' : 'border-white/20 bg-[#09090b] group-hover:border-emerald-500'
                  }`}>
        ...
        <div className="flex items-center gap-2">
          <div className="relative flex-1">
            ...
            <input 
              ...
              className="w-full bg-[#09090b] border border-white/10 rounded-xl pl-8 pr-3 py-2 text-sm text-white focus:outline-none focus:border-emerald-500 transition-colors"
            />
          </div>
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className="flex flex-col gap-2">
          ...
          <label className="flex items-center gap-3 cursor-pointer group" onClick={() => setFilters(prev => ({...prev, inStock: !prev.inStock}))}>
            <div className={`w-4 h-4 rounded-[4px] border flex items-center justify-center transition-colors ${
              filters.inStock ? 'bg-emerald-600 border-emerald-600' : 'border-white/20 bg-[#09090b] group-hover:border-emerald-500'
            }`}>
          ...
          <label className="flex items-center gap-3 cursor-pointer group" onClick={() => setFilters(prev => ({...prev, featured: !prev.featured}))}>
            ...
            <span className={`text-sm ${filters.featured ? 'text-white' : 'text-gray-400 group-hover:text-gray-300'}`}>Destacados</span>
          </label>
      ...
      {activeFiltersCount > 0 && (
        <button 
          ...
          className="mt-4 flex items-center justify-center gap-2 w-full py-3 rounded-xl border border-emerald-500/30 text-emerald-400 hover:bg-emerald-500/10 transition-colors text-sm font-bold"
        >
